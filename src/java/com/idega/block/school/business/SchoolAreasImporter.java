/**
 * @(#)SchoolAreasImporter.java    1.0.0 12:24:13 PM
 *
...
/**
 * <p>Imports all data of {@link SchoolArea}s into {@link CourseProviderArea} 
 * entity.</p>
 * <p>You can report about problems to: 
 * <a href="mailto:martynas@idega.is">Martynas Stakė</a></p>
 *
 * @version 1.0.0 Mar 5, 2014
 * @author <a href="mailto:martynas@idega.is">Martynas Stakė</a>
 */
@Service
@Scope(BeanDefinition.SCOPE_SINGLETON)
public class SchoolAreasImporter extends DefaultSpringBean implements ApplicationListener {

	private SchoolAreaHome schoolAreaHome = null;

	protected SchoolAreaHome getSchoolAreaHome() {
		if (this.schoolAreaHome == null) {
			try {
				this.schoolAreaHome = (SchoolAreaHome) IDOLookup.getHome(SchoolArea.class);
			} catch (IDOLookupException e) {
				getLogger().log(Level.WARNING, 
						"Failed to get " + SchoolAreaHome.class.getSimpleName() + 
						" cause of: ", e);
			}
		}

		return this.schoolAreaHome;
	}
	...
	protected CourseProviderAreaHome getCourseProviderAreaHome() {
		if (this.courseProviderAreaHome == null) {
			try {
				...
				getLogger().log(Level.WARNING, 
						"Failed ot get " + CourseProviderAreaHome.class.getSimpleName() + 
						" cause of: ", e);
			}
		}
		...
		if (event instanceof IWMainSlideStartedEvent) {
			...
			for (SchoolArea schoolArea : schoolAreas) {
				if (!courseProviderAreas.contains(schoolArea)) {
					try {
						CourseProviderArea area = getCourseProviderAreaHome().createIDO();
						area.setAccountingKey(schoolArea.getAccountingKey());
						area.setCategory(schoolArea.getCategory());
						...
						getLogger().info(
								"Stored " + CourseProviderArea.class.getSimpleName() + 
								" for " + SchoolArea.class.getSimpleName() + 
								" by id: '" + schoolArea.getPrimaryKey() + 
								"' and name: '" + schoolArea.getSchoolAreaName() + "'");
					} catch (Exception e) {
						getLogger().log(Level.WARNING, 
								"Failed to create/store " + CourseProviderArea.class.getSimpleName() + 
								" for " + SchoolArea.class.getSimpleName() + 
								" by id: '" + schoolArea.getPrimaryKey() + 
								"' and name: '" + schoolArea.getSchoolAreaName() + 
								"' cause of: ", e);
